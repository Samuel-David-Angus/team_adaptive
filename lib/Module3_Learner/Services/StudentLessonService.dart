...
class StudentLessonService {
  ...
  Future<List<LessonMaterialModel>> getLessonMaterialsByTypeAndStyle(String courseID, String lessonID, String type, String style) async {
    List<LessonMaterialModel> list = [];
    try {
      assert(type == "main" || type == "sub");
      QuerySnapshot<LessonMaterialModel> querySnapshot = await FirebaseFirestore.instance
          .collection("Course")
          .doc(courseID)
          .collection("Lesson")
          .doc(lessonID)
          ...
          toFirestore: (model, _) => model.toJson())
          .get();
      for (DocumentSnapshot<LessonMaterialModel> documentSnapshot in querySnapshot.docs) {
  ...
  Future<LessonMaterialModel?> getLessonMaterialByTypeAndID({required String courseID, required String lessonID, required String type, required String materialID}) async {
    try {
      assert(type == "main" || type == "sub");
