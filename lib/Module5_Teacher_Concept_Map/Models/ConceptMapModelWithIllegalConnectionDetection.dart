class ConceptMapModel {
  ...
  bool willBeProperTree(int from, int to) {
    List<int> prereqsOfPostreq = List.filled(_conceptCount, 0);
    findPreRequisites(to, prereqsOfPostreq);
    if (prereqsOfPostreq[from] == 1) {
      return false;
    }
    List<int> prereqsOfPrereq = List.filled(_conceptCount, 0);
    findPreRequisites(from, prereqsOfPrereq);
    if (prereqsOfPrereq[to] == 1) {
      return false;
    }
    return true;
  }

  bool setConceptConnection(
      String preRequisite, String postRequisite, bool connection) {
    int from = findIndexOfConcept(preRequisite);
    int to = findIndexOfConcept(postRequisite);
    if (connection && !willBeProperTree(from, to)) {
