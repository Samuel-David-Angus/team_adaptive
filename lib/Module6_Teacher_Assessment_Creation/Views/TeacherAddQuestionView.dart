...
class TeacherAddQuestionView extends StatelessWidget {
  ...
  TeacherAddQuestionView({super.key, this.question, required this.lessonModel}) : conceptMap = ConceptMapService().getConceptMap(lessonModel.courseID!), isEditing = question != null;


  @override
  Widget build(BuildContext context) {
    final CreateEditQuestionViewModel addEditViewModel = Provider.of<CreateEditQuestionViewModel>(context);
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text(addEditViewModel.concept != null ? addEditViewModel.concept! : "no concept chosen"),
                  ],
                ),
                const SizedBox(height: 10),
                Row(
                    ...
                    ElevatedButton(
                      onPressed: () {
                        if (addEditViewModel.validateChoice()) {
                          ...
                          showMessageDialog(context, "Choice field must not be empty");
                        }
                    ...
                    onPressed: () async {
                      if (context.mounted) {
                        if (addEditViewModel.validateBeforeSubmitting()) {
                          String userID = teacherQuestionViewModel.authService
                              .userInfo!.id!;
                          ConceptMapModel conceptMapModel = snapshot.data!;
                ...
                content: SingleChildScrollView(
                  child: ListBody(
                    children: concepts.map((item) {
                      return CheckboxListTile(
                        title: Text(item),
                        value: viewModel.concept != null && item == viewModel.concept,
